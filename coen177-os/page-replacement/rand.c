...
// ---------- RAND -----------
int randPage(int cacheSize, int page)
{
	int i = 0, ret = 0;
	...
	if (ptr == NULL)
	{
		...
		else // cache isn't full, insert page
		{
			insertFront(page);
		}
	}
	// else page is in cache, do nothing
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	while((read = getline(&line, &buffersize, stdin)) != -1)
	{
		...
		if(isdigit(line[0]))
		{
			...
			randPage(cacheSize, i); // call LRU function
		}
	}
